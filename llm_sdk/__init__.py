...
class Small_LLM_Model:
    ...
    device: str | None, default=None
        ...
        when available on macOS, ``cuda`` when available, otherwise we fall
        back to ``cpu``. dtype: torch.dtype | None, default=None
        Numerical precision.  When using a GPU or MPS we default to ``float16``
        to keep memory usage reasonable; on CPU we keep ``float32`` for
    ...
    def get_logits_from_input_ids(self, input_ids: list[int]) -> list[float]:
        ...
        # Get logits for the last token in the sequence for the batc
        # (batch size 1)
